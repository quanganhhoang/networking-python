...
import time
import struct
from util import recv_all, eval
...
def handler(conn, addr):
    ...
    data = recv_all(conn, 2) # get number of expressions to process
    ...
    for i in range(num_expression):
        expression_len = int.from_bytes(recv_all(conn, 2), 'big')
        expression = recv_all(conn, expression_len).decode("utf-8")
        result = str(eval(expression)) # evaluate the expression and return a string representation
