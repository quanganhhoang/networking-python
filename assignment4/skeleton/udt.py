...
class NetworkLayer:
  ...
  def _packet_reader(self):
    while not self.stop_accept_pkt:
      # If there is received msg, notify the transport layer first.
      has_msg = False
      with self.buffer_lock:
        if len(self.msg_buffer) > 0:
          has_msg = True
