...
SUPPORTED_ENDPOINT = [EVAL_API, TIME_API, STATUS_API]
...
def validate_request(request_type, url, http_version, body):
    if (url not in SUPPORTED_ENDPOINT and http_version not in SUPPORTED_HTTP_VERSIONS):
        return '404'
    elif url == EVAL_API and not util.validate_expression(body):
        return '400'
    else:
        return '200'
...
def process_request(sock):
    ...
    update_status(url, req_timestamp) # should still be printed in status.html even if invalid
    if (status_code == '404'):
